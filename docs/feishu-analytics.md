...
“卖得好／卖的好／畅销”默认销售金额前10名，显式订单数、前N名优先。“药品”按用户确认的当前商品范围处理，回复标注未按药品类别筛选。指定药名可通过本地对象匹配筛选；仅药品、排除器械等类别条件、销售件数、利润等尚不能执行，不会悄悄丢掉条件。退货和销售出库数量按单位分别统计；库存数量只代表2026-09-16 14:33:41备份时点，可明确选择该时点后查询，不能当今天的实时库存。
...
继续使用 `.local/feishu-app.json`、根目录 `.env` 中已有 `ERP_AI_*` 配置。模型仅接收当前问题、消息日期、业务语义配置、结构化会话状态及服务端裁剪的能力／完整日期元信息，不接收ERP记录、结果、名称目录、群／用户身份或原始对话历史。纯数字选择在应用层绑定动作，内部签名选项不交模型推断展示编号。
