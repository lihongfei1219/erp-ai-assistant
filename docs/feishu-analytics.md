...
“卖得好／卖的好／畅销”默认销售金额前10名，显式订单数、前N名优先。“药品”按用户确认的当前商品范围处理，回复标注未按药品类别筛选。指定药名、仅药品、排除器械、销售件数、利润等尚不能执行，不会悄悄丢掉条件。退货和销售出库数量按单位分别统计；库存数量只代表2026-09-16 14:33:41备份时点，可明确选择该时点后查询，不能当今天的实时库存。
