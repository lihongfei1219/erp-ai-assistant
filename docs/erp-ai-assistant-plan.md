...
已确认：已有 ERP，业务数据库为 SQL Server；平台连接入驻供应商与采购企业，覆盖多个行业；本项目独立建设，前后端分离，后端采用 FastAPI，前端选择 Web + React + TypeScript。当前阶段使用 pandas 作为数据计算框架。第一版服务平台内部管理者与运营人员。周期、指标阈值和验收门槛均为规划建议。第 14 节保留为未来规模扩展参考，当前实施以 pandas 方案及第 15 节为准。

2026-09-16 实施更新：已只读连接 QY 备份还原库 `ERP_Local`，数据所有者确认其覆盖平台全部商家。已建立销售订单分析 CLI、FastAPI 查询接口、前端看板、依赖与测试。结果暂存本地开发快照，助手 SQL Server 库、增量任务和 AI 功能仍属规划。实际范围见 [字段映射](data-dictionary.md)、[指标定义](metrics-v1.md)、[第一阶段验证](phase-1-validation.md) 及 [看板验证](phase-2-validation.md)。

用户已授权按常识采用可调整默认规则。当前以“订单完成、已出库”作为暂定有效销售范围，同时保留全部原始指标；前端包含经营概览、订单明细及业务口径页。所有不确定业务细节集中到 [业务假设台账](business-assumptions.md)，通过规则配置与新快照重算调整。支付退款与商家归属仍需验证。

飞书方向：后续计划每日向飞书中的目标客户推送已完成的数据分析结果。当前仅准备官方 Python SDK `lark-oapi` 及锁定依赖，未实现定时投递、消息内容、接收对象映射、重试或去重，也未发送消息。
...
| 层次 | 方案 | 说明 |
|---|---|---|
| 前端 | 已选择 Web + React + TypeScript，Vite 构建 | 已有看板、表格、趋势图和业务口径；问数与待办待实现 |
...
本地 SQL Server 版本、核心销售表及初步数据规模已核验，前端 Web 形态已确定。后续收集线上接入与部署条件、完整历史、正式用户权限和模型数据出网要求；业务口径按集中台账中的暂定规则继续推进。

**13. 建议项目结构与首批接口**

以下为目标结构；部分后端目录已落地，实际结构以 README 为准：
...
当前已有 FastAPI 和 pandas 销售订单分析代码，已连接本地备份还原库且未修改源数据库。第一阶段采用独立 CLI 计算和本地不可覆盖的 JSON 快照来验证数据与接口；助手 SQL Server 库、任务调度及增量同步尚未实现。该阶段性选择见 [第一阶段安排](implementation-phase-1.md)。
