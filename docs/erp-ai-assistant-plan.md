...
| `backend/config/semantic-catalog.json` | 四域术语、业务口径及语义约定，允许表达尚不能执行的需求 |
| `backend/app/capabilities/` | 现有执行能力注册、参数组合校验及按快照／授权裁剪的能力视图 |
...
复合筛选按每个明确条件分别表达，包含与排除分开；不能依据模型猜测的分类关系将条件视为冗余而省略。商品和客户对象筛选已接入，库存仅支持商品；分类、仓库等其他筛选仍保留并说明能力缺口，不能为了可执行而删除条件。

## 通用任务规划架构重设计（分阶段实施）

2026-09-22，用户要求先重新设计更合理可靠的架构，并明确选用LangGraph。现已完成LangGraph StateGraph主链路迁移、现有能力统一注册及按用户最新优先级接入的P0-3对象绑定，保留供应商无关接口及确定性执行器。通用计算DAG和P0-2逻辑计划契约仍待实施；具体阶段和优先级统一维护在[项目进度](project-status.md#下一步顺序)。
...
已实施目录：`orchestration/`存放LangGraph图、状态、节点、会话恢复和检查点适配，`capabilities/`存放独立能力注册与运行时视图。后续`planning/`承接逻辑计划契约、诊断和执行器适配；`semantic/`保留语言理解与供应商适配；`analysis/`保留事实计算。通过兼容接口渐进迁移，避免一次性重写。
...
第一阶段已落地：`capabilities/registry.py`注册22种现有分析能力（销售6种、退货6种、销售出库6种、库存4种），统一域、操作、指标、语义指标、对象、排序／条数、维度、时间依据、事实粒度、单位与精度及90天／6目标／50项上限。销售概览仍用金额执行计划同时返回订单数，语义指标保留客户对订单数的要求。执行器算法与数值计算未重写。

`capabilities/view.py`将注册表与所选快照、完整日期、质量标记和授权求交；未加载事实、无完整日期、对账失败、授权不足或快照范围不一致时不宣传为可执行。视图不遍历业务明细代替完整对账，执行时仍须通过原有完整证据和业务事实校验。模型、`/analysis/catalog`与飞书帮助共享该视图；语义编译、引导建议和`AnalysisStep`从注册矩阵校验支持组合。

业务词汇中的利润、退货率、库存趋势等仍可被理解并保留，但不成为可执行能力。配置中的`executable_*`与执行默认指标由注册表生成；能力结构指纹并入语义版本，当前能力覆盖并入快照绑定。版本或覆盖变化后旧会话需重新开始／显式恢复。注册表已开放按域区分的商品／客户对象筛选；复杂布尔条件、任意计算依赖及跨域关联仍未开放。
