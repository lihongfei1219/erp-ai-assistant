...
2026-09-22，用户要求先重新设计更合理可靠的架构，再推进对象筛选，并明确选用LangGraph。现已完成LangGraph StateGraph主链路迁移与现有能力统一注册，保留供应商无关接口及确定性执行器。通用计算DAG和对象绑定仍是后续设计；已实施边界见下方编排及注册表说明。具体阶段和优先级统一维护在[项目进度](project-status.md#下一步顺序)。
...
业务词汇中的利润、退货率、库存趋势等仍可被理解并保留，但不成为可执行能力。配置中的`executable_*`与执行默认指标由注册表生成；能力结构指纹并入语义版本，当前能力覆盖并入快照绑定。版本或覆盖变化后旧会话需重新开始／显式恢复。新增条件筛选、任意计算依赖、合法关联键与关系基数仍属于下一阶段，当前不开放这些执行能力。
