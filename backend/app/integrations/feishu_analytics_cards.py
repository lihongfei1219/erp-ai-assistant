...
from app.analysis.analytics import TITLES
...
def _period(step) -> str:
    ...
    if step.kind in RANKINGS:
        metric = "销售金额" if step.metric == "amount" else "订单数"
        value += f"\n按{metric}从高到低 · 前 {step.top_n} 名"
...
def _card(response: AnalysisResponse, row_limit: int, charts: bool) -> dict:
    ...
    for index, (step, result) in enumerate(zip(response.plan.steps, response.results, strict=True)):
        ...
        if len(response.results) > 1:
            elements.append(
                text(f"{index + 1}. {TITLES.get(result.kind, '分析结果')}", size="heading")
            )
    ...
    explanation = [
        ...
        "纳入状态：" + "、".join(p.included_statuses) + "；金额非支付成交额，未扣退款。",
    ...
    title = (
        TITLES.get(response.results[0].kind, "分析结果")
        if len(response.results) == 1
        else "AI 数据分析"
    )
    return {
